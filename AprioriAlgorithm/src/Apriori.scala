...
object Apriori {
 def main(arg:Array[String]): Unit ={
    var config_file = Source.fromFile("C:\Users\Iqra Malik\workspace\config.txt" ).getLines.toList
    var t_items = config_file(0).toInt + 1
    var t_trans = config_file(1).toInt + 1
    var min_sup = config_file(2).toInt
    
    var trans_file = Source.fromFile("C:\Users\Iqra Malik\workspace\trans.txt" ).getLines.toList.map(f => f.split(",")).toArray.map(x => x.map(f=> f.toInt))
